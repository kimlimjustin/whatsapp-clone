...
import { getUserByToken, getGroupByCode, getGroupById, getUserById, getAllUser } from '../Service';
            ...
            {width > 900 ?
                <div>
                    ...
                    <SideNavBar
                        ...
                        friends={friends}
                        groups={groups}
                        inputGroupMembers={inputGroupMembers}
                                                    ...
                                                    if (user in friends && !targetGroup.member.includes(friends[user].email)) {
                                                        return <div key={user} className="form-group">
                                                            <input type="checkbox" id={friends[user] && `${friends[user].email}-new`} value={friends[user] && friends[user].email}
                                                                onChange={({ target: { checked, value } }) => {
                                                                    if (checked) setInputNewMember(prev => [...prev, value]);
